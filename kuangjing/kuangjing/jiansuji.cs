...
namespace kuangjing
{
    public partial class jiansuji : CCSkinMain
    {
        public jiansuji()
        {
            InitializeComponent();
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (id.Equals("") == false)
                    {
                        ...
                        {
                            chart1.Series["变量"].LegendText = "减速机数据值";

                            //                //设置坐标轴名称
                            chart1.ChartAreas["ChartArea1"].AxisX.Title = shebei1 + "号减速机";
                            ...
                            string sql = "select * from  (select sensordata.Sid,dsensor.Sname,sensordata.`value`,sensordata.time from sensordata,dsensor where sensordata.workid=" + id + " and sensordata.Eid = " + shebei1 + " and sensordata.Sid=dsensor.Sid order by time desc)a group by Sid";
        ...
        private void getFactory(object obj)
        {
            try
            {
                ...
                //string workid1 = selectID(comboBox3.Text);
                //string sql = "select * from equipment where Etype =2 and workid=" + "'" + workid1 + "'";
               
                //MySqlDataReader rd = msc.getDataFromTable(sql);
                //while (rd.Read())
                //{
                //    comboBox2.Items.Add(rd["Eid"].ToString());
                //}
                //rd.Close();
                //msc.Close();
